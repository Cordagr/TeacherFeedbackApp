...
classroomRouter.post('/postAnonymousFeedback', async (req, res) => {
  const { classroomId, feedback, user } = req.body;
  try {
    ...
    session.feedback.push(feedbackData);
    await session.save();

    // Emit real-time feedback using Socket.IO
    const io = req.app.get('io');
    if (io) {
      io.to(classroomId).emit('newFeedback', {
        ...feedbackData,
...
classroomRouter.post('/sendMessage', async (req, res) => {
  ...
  try {
    ...
    const io = req.app.get('io');
    if (io) {
      io.to(classroomId).emit('newMessage', {
        ...messageData,
        id: Date.now() + Math.random()
      });
    }
...
classroomRouter.get('/getSessionFeedback/:classroomId', async (req, res) => {
  const { classroomId } = req.params;
  try {
    const session = await ClassroomSession.findOne({ classroomId, isOpen: true });
    if (!session) {
    ...
    res.status(200).json({ feedback: session.feedback || [] });
  } catch (error) {
...
classroomRouter.get('/getSessionMessages/:classroomId', async (req, res) => {
  const { classroomId } = req.params;
  try {
    const session = await ClassroomSession.findOne({ classroomId, isOpen: true });
    if (!session) {
